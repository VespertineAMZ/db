...
module.exports = {
	name: 'levels',
	description: 'lists all characters and their xp, and sorts using xp.',
	aliases: ['charlevels', 'xps'],
	async execute(message, args) {
		...
		for(i = 0; i < count2; i ++) {
			...
			const charList = await Profiles.find({}).skip(pageOffset).limit(pageLimit).sort([ ['experience', 'descending'] ]);
			const charString = charList.map(t => `⟢ ${t.name} ➺ ${t.experience}`).join('\n ') || 'No characters set.';
