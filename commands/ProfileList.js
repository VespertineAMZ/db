...
module.exports = {
	name: 'profilelist',
	description: 'list all of the characters and format them.',
	aliases: ['charlist', 'listchar', 'charalist', 'chars', 'charas'],
	async execute(message, args) {
		...
		for(i = 0; i < count2; i ++) {
			...
			const charList = await Profiles.find({}).skip(pageOffset).limit(pageLimit).sort([ ['name', 'ascending'] ]);
			const charString = charList.map(t => t.name).join('\n ') || 'No characters set.';
